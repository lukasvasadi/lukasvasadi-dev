...
async function handle_error_and_jsonify(event, options2, error2) {
	...
	return (
		(await options2.hooks.handleError({ error: error2, event })) ?? {
			message:
				event.route.id === null && error2 instanceof NotFound
					? 'Not Found'
					: 'Internal Error'
		}
	)
	...
	for (const key2 in object) {
		if (typeof object[key2]?.then === 'function') {
			return Object.fromEntries(
				await Promise.all(
					Object.entries(object).map(async ([key3, value]) => [key3, await value])
				)
			)
		}
	}
...
class BaseProvider {
	...
	get_header(is_meta = false) {
		...
		for (const key2 in directives) {
			if (
				is_meta &&
				(key2 === 'frame-ancestors' || key2 === 'report-uri' || key2 === 'sandbox')
			) {
				continue
			}
			const value =
...
`)
	...
	try {
		const strings = nodes.map((node) => {
			if (!node) return 'null'
			return `{"type":"data","data":${devalue.uneval(node.data, replacer)},${stringify_uses(
				node
		...
		return options2.hooks.handleFetch({
			...
			fetch: async (info2, init3) => {
				...
				if (info2 !== original_request) {
					...
					credentials =
						(info2 instanceof Request ? info2.credentials : init3?.credentials) ??
						'same-origin'
				}
				if (
		...
		if (!this.#options.hooks) {
			try {
				...
				this.#options.hooks = {
					...
					handleError:
						module.handleError || (({ error: error2 }) => console.error(error2)),
					handleFetch:
						module.handleFetch || (({ request, fetch: fetch2 }) => fetch2(request))
				}
			} catch (error2) {
				{
					throw error2
